...
#Disabling Flagship apps so A32 don't lag too much when using Tab port
...
FIX_CAMERA "$FIRM_DIR/$TARGET_DEVICE"
FIX_BLUETOOTH "$FIRM_DIR/$TARGET_DEVICE"
...
BUILD_PROP "$FIRM_DIR/$TARGET_DEVICE" "system" "ro.build.display.id" "${B_ID} ${B_V}"
BUILD_PROP "$FIRM_DIR/$TARGET_DEVICE" "product" "ro.build.display.id" "${B_ID} ${B_V}"
BUILD_PROP "$FIRM_DIR/$TARGET_DEVICE" "system" "ro.product.system.model" "$TARGET_DEVICE"
